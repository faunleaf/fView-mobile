<template>
  <div>
    <i-popup-picker :data="list" :title="title" :display-format="cellFormat" v-model="currentValue" :inline-desc="inlineDesc" :placeholder="placeholder" @on-hide="emitHide" @on-show="$emit('on-show')" :value-text-align="valueTextAlign" :column-width="[1/2, 1/6]"></i-popup-picker>
  </div>
</template>

<script>
import value2name from '../../filters/value2name'
import iPopupPicker from '../popup-picker'
import getDateRange from '../../tools/date/range'
import getNumberRange from '../../tools/number/range'

export default {
  components: { iPopupPicker },
  props: {
    title: {
      type: String,
      required: true
