...
popupDialog.prototype.show = function () {
  if (this.params.showMask) {
    ...
    if (this.container.style['zIndex'])
      this.mask.style['zIndex'] = this.container.style['zIndex'] - 1;
    else
...
popupDialog.prototype.hide = function (shouldCallback = true) {
  ...
  if (!document.querySelector('.vux-popup-dialog.vux-popup-show')) {
    ...
    var zIndex = 0;
    document.querySelectorAll('.vux-popup-dialog.vux-popup-show').forEach((item) => {
      if (item.style['zIndex'] && item.style['zIndex'] > zIndex)
        zIndex = item.style['zIndex'];
    });
    setTimeout(() => {
      this.mask && /show/.test(this.mask.className) && (this.mask.style['zIndex'] = zIndex - 1)
    }, 400)
  }
